...
function hideScreen(screenId) {
  ...
  function showMatteo() {
    hideScreen("home-screen")
      .then(() => showScreenZoomIn("matteo-screen"))
      .then(updateTimeHowMutchTimeMatteoAlive); // Atualiza o contador após a animação
  }
  ...
  function updateTime() {
    const startDate = new Date("0000-00-00"); // Defina a data de início
    const now = new Date();
  ...
  function updateTimeHowMutchTime() {
    const startDate = new Date("2025-02-16"); // Defina a data de início
    const now = new Date();
    const diff = now - startDate;
  ...
  setInterval(updateTimeHowMutchTimeMatteoAlive, 1000);
